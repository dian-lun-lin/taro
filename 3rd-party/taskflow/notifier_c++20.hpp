...
class Notifier {
 ...
 private:
  ...
  void _park(Waiter* w) {
    unsigned nos = Waiter::kNotSignaled;
    w->state.compare_exchange_weak(nos, Waiter::kWaiting);
    w->state.wait(Waiter::kWaiting);
  }
  ...
  void _unpark(Waiter* waiters) {
    ...
    for (Waiter* w = waiters; w; w = next) {
      ...
      if(w->state.exchange(Waiter::kSignaled) == Waiter::kWaiting) {
        w->state.notify_one();
      }
    }
  }
